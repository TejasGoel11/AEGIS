...
class AegisCore {
  constructor(canvas, { compact = false, architecture = false } = {}) {
    ...
    this.angle = -.65;
    this.tilt = .49/* Small local 3D renderer. No CDN, fonts, or external services required. */
class AegisCore {
  constructor(canvas, { compact = false, architecture = false } = {}) {
    this.canvas = canvas;
    this.ctx = canvas.getContext('2d');
  ...
  paint() {
    ...
    aura.addColorStop(0, `rgba(30,63,203,${.08 + this.energy * .07})`);
    aura.addColorStop(.5, 'rgba(85,128,255,.045)'); aura.addColorStop(1, 'rgba(0,0,0,0)');
    ...
    if (!this.compact) {
      for (let n = -5; n <= 5; n++) {
        line([this.project(n * 42, 145, -210), this.project(n * 42, 145, 210)], '#dbe7ff17', .6);
        line([this.project(-210, 145, n * 42), this.project(210, 145, n * 42)], '#dbe7ff17', .6);
      }
      const orbit = [];
      for (let n = 0; n <= 100; n++) { const t = n / 100 * Math.PI * 2; orbit.push(this.project(Math.cos(t) * 205, 136, Math.sin(t) * 205)); }
      line(orbit, '#8ba5ff66', .8);
    }
    const faces = [];
    ...
    const colors = {
      base:['#0b1220','#182440'], edge:['#11255f','#1e3fcb'], metal:['#101c32','#2e4a7f'],
      active:['#183998','#315bf0'], cap:['#1c3159','#4e6eb4'], dark:['#0b1220','#182a4d'],
      copper:['#183898','#2354e0'], glow:['#3266ff','#b7cbff'], chip:['#1e3fcb','#6f94ff'],
      vent:['#070d17','#243b67'], screw:['#2e4775','#a9c0ff']
    };
    this.hitFaces = [];
    faces.sort((a,b) => a.depth - b.depth).forEach(face => {
      ...
      if (face.material === 'glow') { ctx.shadowColor = '#7da2ff'; ctx.shadowBlur = 8 + this.energy * 9; }
      ctx.fill(); ctx.shadowBlur = 0;
      ctx.strokeStyle = top ? '#dbe7ff4d' : '#dbe7ff1e'; ctx.lineWidth = .6; ctx.stroke();
    ...
    if (!this.compact) {
      for (let n = 0; n < 5; n++) {
        ...
        ctx.beginPath(); ctx.arc(p[0],p[1],2,0,Math.PI*2); ctx.fillStyle='#dbe7ff';ctx.shadowColor='#7da2ff';ctx.shadowBlur=10;ctx.fill();ctx.shadowBlur=0;
      }
    }
